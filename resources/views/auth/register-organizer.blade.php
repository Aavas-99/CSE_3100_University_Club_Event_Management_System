...
<head>
    ...
    <title>Register (Club Rep) | KUET EMS</title>
    ...
    <div class="relative w-full max-w-2xl mx-auto">
        ...
        <div class="bg-white/5 backdrop-blur-xl border border-white/10 rounded-3xl p-8 shadow-2xl">
            ...
            <div class="text-center mb-8">
                ...
                <h1 class="text-2xl font-bold text-white">Club Representative Registration</h1>
                <p class="text-slate-400 text-sm mt-1">Create your club management account</p>
            </div>
            ...
            <form method="POST" action="{{ route('register') }}" class="space-y-5">
                    ...
                    <div>
                        ...
                        <div class="relative">
                            <select name="department" required
                                class="w-full px-4 py-3 rounded-xl bg-white/5 border border-white/10 text-white text-sm focus:outline-none focus:border-kuet-500 focus:ring-1 focus:ring-kuet-500 transition-all appearance-none cursor-pointer">
                                @foreach(['CSE','EEE','CE','ME','ECE','IEM','ESE','BME','URP','LE','TE','BECM','ARCH','MSE','CHE','MTE'] as $dept)
                ...
                <button type="submit" class="w-full py-3 rounded-xl bg-gradient-to-r from-kuet-600 to-kuet-700 text-white font-semibold text-sm hover:from-kuet-700 hover:to-kuet-800 transition-all shadow-lg shadow-kuet-500/25 flex items-center justify-center gap-2 mt-6">
                    ...
                    Create Club Representative Account
                </button>
            </form>

            <div class="mt-6 pt-6 border-t border-white/10 text-center">
                <p class="text-sm text-slate-400">
                    Already have an account? <a href="{{ route('login.organizer') }}" class="text-kuet-400 font-semibold hover:text-kuet-300 transition-colors">Sign in</a>
                </p>
            </div>
        </div>
    </div>
