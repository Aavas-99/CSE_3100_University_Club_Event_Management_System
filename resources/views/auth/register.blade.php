    ...
    <div class="relative w-full max-w-2xl mx-auto">
        ...
        <div class="bg-white/5 backdrop-blur-xl border border-white/10 rounded-3xl p-8 shadow-2xl">
            ...
            <div class="text-center mb-8">
                ...
                <p class="text-slate-400 text-sm mt-1">Register as a student or club organizer</p>
            </div>
            ...
            <form method="POST" action="{{ route('register') }}" class="space-y-5">
                ...
                <div class="grid md:grid-cols-2 gap-5">
                    ...
                    <div>
                        <label class="block text-sm font-medium text-slate-300 mb-2">Student ID</label>
                    ...
                    <div>
                        ...
                        <div class="relative">
                            <select name="department" class="w-full px-4 py-3 rounded-xl bg-white/5 border border-white/10 text-white text-sm focus:outline-none focus:border-kuet-500 focus:ring-1 focus:ring-kuet-500 transition-all appearance-none cursor-pointer">
                                @foreach(['EEE','CSE','CE','ME','ECE','IEM','ESE','BME','URP','LE','TE','BECM','ARCH','MSE','CHE','MTE'] as $dept)
                                    <option value="{{ $dept }}" class="bg-slate-800">{{ $dept }}</option>
                                @endforeach
                            </select>
                    ...
                    <div>
                        ...
                        <div class="relative">
                            <select name="role" required class="w-full px-4 py-3 rounded-xl bg-white/5 border border-white/10 text-white text-sm focus:outline-none focus:border-kuet-500 focus:ring-1 focus:ring-kuet-500 transition-all appearance-none cursor-pointer">
                                <option value="student" class="bg-slate-800">Student</option>
                                <option value="organizer" class="bg-slate-800">Club Organizer</option>
                            </select>
                            <i class="fas fa-chevron-down absolute right-4 top-1/2 -translate-y-1/2 text-slate-500 text-xs pointer-events-none"></i>
                        </div>
                    </div>
                ...
                <button type="submit" class="w-full py-3 rounded-xl bg-gradient-to-r from-kuet-600 to-kuet-700 text-white font-semibold text-sm hover:from-kuet-700 hover:to-kuet-800 transition-all shadow-lg shadow-kuet-500/25 flex items-center justify-center gap-2 mt-6">
                    <i class="fas fa-user-plus"></i>
                    Create Account
                </button>
            </form>
